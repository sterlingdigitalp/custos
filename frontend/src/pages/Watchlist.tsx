...
export default function Watchlist() {
  ...
  return <>
    ...
    {products === null && !error ? <LoadingBlock label="Loading watchlist" /> : <div className="panel overflow-hidden"><div className="overflow-x-auto"><table className="min-w-full divide-y divide-divider"><thead className="table-head"><tr><th className="px-4 py-3">Product</th><th className="px-4 py-3">ASIN</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Sales rank</th><th className="px-4 py-3">Offers</th><th className="px-4 py-3">7-day price</th><th className="px-4 py-3">Alerts</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-divider bg-surface">{products?.filter(p => !p.isArchived).map(product => <tr key={product.id} className="hover:bg-base/30"><td className="px-4 py-3"><div className="flex min-w-64 items-center gap-3">{product.imageUrl ? <img src={product.imageUrl} alt="" className="h-12 w-12 rounded-xl bg-base object-contain" /> : <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-base text-xs text-text-muted">No image</div>}<Link className="font-semibold text-text-primary hover:text-accent" to={`/p/${product.asin}`}>{product.title ?? 'Awaiting catalog data'}</Link></div></td><td className="px-4 py-3 font-mono text-xs text-text-secondary">{product.asin}</td><td className="px-4 py-3 font-semibold text-text-primary">{money(product.buyBoxPrice ?? product.lowestNewPrice)}</td><td className="px-4 py-3">{integer(product.salesRank)}</td><td className="px-4 py-3">{integer(product.offerCount)}</td><td className="px-4 py-2"><Sparkline asin={product.asin} /></td><td className="px-4 py-3">{counts[product.asin] ? <span className="rounded-full bg-accent/10 px-2 py-1 text-xs font-bold text-accent">{counts[product.asin]}</span> : '—'}</td><td className="px-4 py-3 text-right"><button className="text-xs font-semibold text-text-secondary hover:text-metric-down" onClick={() => void archive(product.id)}>Archive</button></td></tr>)}</tbody></table></div>{products?.filter(p => !p.isArchived).length === 0 && <EmptyState title="Your watchlist is empty" detail="Add one or more ASINs above, then run a sweep." />}</div>}
  </>
}
