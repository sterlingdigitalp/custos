...
export default function FinderPage() {
  ...
  return <><PageHeader title="Finder" description="Screen the products in your tracked corpus using current metrics and historical deltas." />
    ...
    <div className="panel overflow-hidden"><div className="overflow-x-auto"><table className="min-w-full divide-y"><thead className="table-head"><tr><th className="px-4 py-3">{head('title', 'Product')}</th><th className="px-4 py-3">{head('currentPrice', 'Price')}</th><th className="px-4 py-3">{head('salesRank', 'Rank')}</th><th className="px-4 py-3">{head('offerCount', 'Offers')}</th><th className="px-4 py-3">{head('priceDropPercent', 'Price drop')}</th><th className="px-4 py-3">{head('rankImprovedPercent', 'Rank improved')}</th></tr></thead><tbody className="divide-y">{sorted.map(row => <tr key={row.asin}><td className="px-4 py-3"><Link className="font-semibold text-custos-700 hover:underline" to={`/p/${row.asin}`}>{row.title ?? row.asin}</Link><div className="font-mono text-xs text-slate-400">{row.asin}</div></td><td className="px-4 py-3 font-semibold">{money(row.currentPrice)}</td><td className="px-4 py-3">{integer(row.salesRank)}</td><td className="px-4 py-3">{integer(row.offerCount)}</td><td className="px-4 py-3">{percent(row.priceDropPercent)}</td><td className="px-4 py-3">{percent(row.rankImprovedPercent)}</td></tr>)}</tbody></table></div>{results === null ? <EmptyState title="Ready to search" detail="Set any combination of filters and run the finder." /> : results.length === 0 && <EmptyState title="No matches" detail="Try relaxing one or more filters." />}</div>
  </>
}
