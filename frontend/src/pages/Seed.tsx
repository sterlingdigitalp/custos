...
export default function SeedPage() {
  ...
  return <><PageHeader title="Seed the corpus" description="Search Amazon’s catalog by keyword, review candidates, and add the useful ones to tracking." />
    ...
    {queries.length > 0 && <div className="mb-5 flex flex-wrap items-center gap-2"><span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Saved searches</span>{queries.map(saved => <button key={saved.id} className="rounded-full border bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-custos-300 hover:text-custos-700" title={saved.lastRunAt ? `Last run ${new Date(saved.lastRunAt).toLocaleString()}` : 'Not run yet'} onClick={() => void runSearch(saved.query, saved)}>{saved.query} ↻</button>)}</div>}
    ...
    {items === null ? <div className="panel"><EmptyState title="Find products to track" detail="Your saved keyword searches will remain available for quick re-runs." /></div> : items.length === 0 ? <div className="panel"><EmptyState title="No catalog results" detail="Try a broader keyword or a mock catalog fixture term." /></div> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{items.map(item => <label key={item.asin} className={`panel relative flex cursor-pointer gap-4 p-4 ${selected.has(item.asin) ? 'ring-2 ring-custos-500' : ''} ${item.isTracked ? 'cursor-default opacity-70' : ''}`}><input type="checkbox" className="mt-1 h-4 w-4 accent-custos-600" checked={selected.has(item.asin)} disabled={item.isTracked} onChange={() => setSelected(current => { const next = new Set(current); next.has(item.asin) ? next.delete(item.asin) : next.add(item.asin); return next })} />{item.imageUrl ? <img src={item.imageUrl} alt="" className="h-24 w-24 rounded-lg border object-contain" /> : <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-xs text-slate-400">No image</div>}<div className="min-w-0"><div className="line-clamp-2 font-semibold text-ink">{item.title}</div><div className="mt-1 font-mono text-xs text-slate-400">{item.asin}</div><div className="mt-2 text-xs text-slate-600">Rank {integer(item.salesRank)}{item.price != null && ` · ${money(item.price)}`}</div>{item.isTracked && <span className="mt-2 inline-block rounded-full bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700">Already tracked</span>}</div></label>)}</div>}
  </>
}
