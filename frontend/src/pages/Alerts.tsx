...
function eventTone(message: string) { return /buy box.*won/i.test(message) ? 'text-accent' : /buy box.*lost/i.test(message) ? 'text-text-secondary' : 'text-text-primary' }
...
export default function AlertsPage() {
  ...
  return <><PageHeader title="Alerts" description="Manage monitoring rules and review every triggered event." />{error && <div className="mb-4"><ErrorBlock message={error} retry={() => void load()} /></div>}
    <section className="panel mb-6 overflow-hidden"><div className="border-b border-divider px-5 py-4"><h2 className="font-bold text-text-primary">Rules</h2></div>{alerts === null ? <LoadingBlock label="Loading rules" /> : <div className="overflow-x-auto"><table className="min-w-full divide-y divide-divider"><thead className="table-head"><tr><th className="px-4 py-3">Product</th><th className="px-4 py-3">Rule</th><th className="px-4 py-3">Last fired</th><th className="px-4 py-3">Active</th><th className="px-4 py-3"></th></tr></thead><tbody className="divide-y divide-divider">{alerts.map(alert => <tr key={alert.id} className="hover:bg-base/30"><td className="px-4 py-3"><Link className="font-semibold text-accent hover:underline" to={`/p/${alert.asin}`}>{titles[alert.asin] ?? alert.asin}</Link><div className="font-mono text-xs text-text-muted">{alert.asin}</div></td><td className="px-4 py-3">{ruleSummary(alert)}</td><td className="px-4 py-3 text-text-muted">{localDate(alert.lastFiredAt)}</td><td className="px-4 py-3"><Toggle checked={alert.isActive} label={`Toggle ${alert.asin} alert`} onChange={value => void toggle(alert, value)} /></td><td className="px-4 py-3 text-right"><button className="text-xs font-semibold text-metric-down" onClick={() => void remove(alert.id)}>Delete</button></td></tr>)}</tbody></table>{alerts.length === 0 && <EmptyState title="No alert rules" detail="Create one from a product chart." />}</div>}</section>
    <section className="panel overflow-hidden"><div className="flex items-center justify-between border-b border-divider px-5 py-4"><div><h2 className="font-bold text-text-primary">Events inbox</h2><p className="text-xs text-text-muted">{events.filter(e => !e.isRead).length} unread</p></div><button className="btn-secondary" disabled={!events.some(e => !e.isRead)} onClick={() => void mark(events.filter(e => !e.isRead).map(e => e.id))}>Mark all read</button></div><div className="divide-y divide-divider">{events.map(event => <div key={event.id} className={`flex flex-col gap-3 p-4 sm:flex-row sm:items-center ${event.isRead ? 'bg-surface' : 'bg-accent/5'}`}><div className={`h-2 w-2 shrink-0 rounded-full ${event.isRead ? 'bg-divider' : 'bg-accent'}`} /><div className="min-w-0 flex-1"><div className={`font-medium ${eventTone(event.message)}`}>{event.message}</div><div className="mt-1 text-xs text-text-muted"><Link to={`/p/${event.asin}`} className="font-mono hover:text-accent hover:underline">{event.asin}</Link> · {localDate(event.ts)} · {event.delivered ? 'Notification delivered' : event.deliveryError ?? 'Web inbox only'}</div></div>{!event.isRead && <button className="btn-secondary" onClick={() => void mark([event.id])}>Mark read</button>}</div>)}{events.length === 0 && <EmptyState title="Inbox is clear" detail="Triggered alerts will appear here." />}</div></section>
  </>
}
