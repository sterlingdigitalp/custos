...
export default function Sparkline({ asin }: { asin: string }) {
  ...
  return <svg width="100" height="30" viewBox="0 0 100 30" role="img" aria-label={`Seven-day price trend for ${asin}`}><path d="M2 27 H98" stroke="#23302A" fill="none" />{path ? <path d={path} stroke="#BEF264" strokeWidth="2" fill="none" strokeLinejoin="round" /> : <text x="50" y="19" textAnchor="middle" fontSize="9" fill="#4A5A53">No trend</text>}</svg>
}
